...
def open_configuration(
    ...
    kld_coefficient,
    orth_te_coefficient,
    orth_unet_coefficient,
    freeze_network_1,
    semi_freeze_network_1,
    anneal_epoch,
    half_annel_epoch
):
    # Get list of function parameters and values
...
def train_model(
    ...
    if pretrained_model_name_or_path_2 == '':
        msgbox('Source model 2 information is missing')
        return

    if train_data_dir == '':
        msgbox('Image folder path of network1 is missing')
        return
    
    if train_data_dir_2 == '':
        msgbox('Image folder path of network2 is missing')
        return

    if not os.path.exists(train_data_dir):
        msgbox('Image folder of network1 does not exist')
        return

    if not os.path.exists(train_data_dir_2):
        msgbox('Image folder of network2 does not exist')
        return

    if reg_data_dir != '':
        if not os.path.exists(reg_data_dir):
            msgbox('Regularisation folder of network1 does not exist')
            return

    if reg_data_dir_2 != '':
        if not os.path.exists(reg_data_dir_2):
            msgbox('Regularisation folder of network2 does not exist')
            return
    ...
    run_cmd += (
        f' --pretrained_model_name_or_path_2="{pretrained_model_name_or_path_2}"'
    )
    ...
    # add image folder of network_2
    run_cmd += f' --train_data_dir="{train_data_dir_2}"'
    if len(reg_data_dir_2):
        run_cmd += f' --reg_data_dir="{reg_data_dir_2}"'
    ...
    # lora weights of network2
    if not lora_network_weights_2 == "":
        run_cmd += f' --network_weights_2="{lora_network_weights}"'
    ...
    if not kld_coefficient == '':
        run_cmd += f' --kld_coefficient="{kld_coefficient}"'
    if not orth_te_coefficient == '':
        run_cmd += f' --orth_te_coefficient="{orth_te_coefficient}"'
    if not orth_unet_coefficient == '':
        run_cmd += f' --orth_unet_coefficient="{orth_unet_coefficient}"'
    if not freeze_network_1 == '':
        run_cmd += f' --freeze_network_1="{freeze_network_1}"'
    if not semi_freeze_network_1 == '':
        run_cmd += f' --semi_freeze_network_1="{semi_freeze_network_1}"'
    if not anneal_epoch == '':
        run_cmd += f' --anneal_epoch="{anneal_epoch}"'
    if not half_annel_epoch == '':
        run_cmd += f' --half_annel_epoch="{half_annel_epoch}"'
...
def UI(**kwargs):
    ...
    interface = gr.Blocks(css=css, title='Kohya_ss lora_dualnetwork GUI', theme=gr.themes.Default())
    with interface:
        with gr.Tab("LoRA Dualnetwork"):
            (   
    ...
    with gr.Tab('Folders'):
        with gr.Row():
            train_data_dir = gr.Textbox(
                label='Image folder 1',
                placeholder='Folder where the training folders containing the images of network1 are located',
            )
            ...
            reg_data_dir = gr.Textbox(
                label='Regularisation folder 1',
                placeholder='(Optional) Folder where where the regularization folders containing the images of network1 are located',
            )
        ...
        with gr.Row():
            train_data_dir_2 = gr.Textbox(
                label='Image folder 2',
                placeholder='Folder where the training folders containing the images of network2 are located',
            )
            train_data_dir_folder_2 = gr.Button('📂', elem_id='open_folder_small')
            train_data_dir_folder_2.click(
                get_folder_path,
                outputs=train_data_dir_2,
                show_progress=False,
            )
            reg_data_dir_2 = gr.Textbox(
                label='Regularisation folder 2',
                placeholder='(Optional) Folder where where the regularization folders containing the images of network2 are located',
            )
            reg_data_dir_folder_2 = gr.Button('📂', elem_id='open_folder_small')
            reg_data_dir_folder_2.click(
                get_folder_path,
                outputs=reg_data_dir_2,
                show_progress=False,
            )
        with gr.Row():
        ...
        train_data_dir_2.change(
            remove_doublequote,
            inputs=[train_data_dir_2],
            outputs=[train_data_dir_2],
        )
        reg_data_dir_2.change(
            remove_doublequote,
            inputs=[reg_data_dir_2],
            outputs=[reg_data_dir_2],
        )
        output_dir.change(
            remove_doublequote,

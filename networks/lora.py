...
class LoRAModule(torch.nn.Module):
    ...
    def __init__(self, lora_name, org_module: torch.nn.Module, multiplier=1.0, lora_dim=4, alpha=1):
        ...
        self.hidden_dict = {} # add for gradients preservation.
    ...
    def forward(self, x):
        if self.region is None:
            ori = self.org_forward(x)
            add_lora = self.lora_up(self.lora_down(x)) * self.multiplier * self.scale
            hidden_states = ori + add_lora
            # if self.lora_name == "lora_unet_up_blocks_3_attentions_2_transformer_blocks_0_ff_net_2":
            self.hidden_dict[self.lora_name] = add_lora
